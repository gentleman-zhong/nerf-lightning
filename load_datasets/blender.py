...
from load_datasets.ray_utils import *
...
#                                              self.far*torch.ones_like(rays_o[:, :1])],
#                                              1)] # (h*w, 8)
#             # 得到测试集或者验证集所有光线[训练集图片数量*H*W,8(光线起始点，像素点，near，far)]
#             self.all_rays = torch.cat(self.all_rays, 0) # (len(self.meta['frames])*h*w, 3)
#             # 所有光线对应的rgb值
#             self.all_rgbs = torch.cat(self.all_rgbs, 0) # (len(self.meta['frames])*h*w, 3)
#
...
if __name__ == '__main__':
    datasets = BlenderDataset(root_dir='../data/nerf_synthetic/lego')
    datasets_valid = BlenderDataset(root_dir='../data/nerf_synthetic/lego', split='val')
    # 训练集返回一条光线
    print(datasets[1])
    # 如果不是训练集，返回的是一幅图
    print(datasets_valid[1])